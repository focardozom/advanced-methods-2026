...
```{r load-data}
load(here("causal", "small_data.rda"))

small_data <- small_data |> drop_labels()
...
data_2019 <- small_data %>%
    ...
    mutate(
        marijuana = case_when(marijuana == 1 ~ "No", marijuana == 2 ~ "Yes")
    ) |>
    mutate(alcohol = case_when(alcohol == 1 ~ "No", alcohol == 2 ~ "Yes"))

data_2019 |> count(marijuana)
...
data_2019 %>%
    ggplot(aes(age, fill = factor(alcohol))) +
...
data_2019 %>%
    ...
    group_by(alcohol) %>%
    summarise(
        `Females proportion` = mean(as.numeric(as.character(sex)), na.rm = T)
    ) %>%
...
data_2019 %>%
    ...
    filter(!is.na(alcohol)) %>%
    pivot_longer(cols = -alcohol, names_to = "Drugs", values_to = "Use") %>%
...
*No design; use all the information Yes and No.*
...
glm(factor(marijuana) ~ alcohol, data = data_2019, family = "binomial") %>%
...
to_ipw <- bind_cols(to_ipw, data_2019) %>%
    select(.pred_Yes, alcohol, everything())
...
to_ipw |>
    filter(!is.na(alcohol)) |>
    mutate(
        pred_class = case_when(.pred_Yes > 0.5 ~ 1, .pred_Yes <= 0.5 ~ 0)
    ) %>%
    table1(~ pred_class | alcohol, data = .)
...
ipw_test_recipe <- recipe(
    alcohol ~ age + sex + q30 + q50 + q51 + q52 + q53 + q54 + q55 + q56 + ipw,
    data = data_ipw
)
...
causal_effect_recipe <-
    recipe(marijuana ~ ., data = finaldata)
...
What if we compare our results to a model with controls?

```{r}
glm(
    factor(marijuana) ~ alcohol +
        age +
        sex +
        q30 +
        q50 +
...
![https://www.publichealth.columbia.edu/research/population-health-methods/difference-difference-estimation](img/did_imagen.png)
...
diff_diff <- small_data %>%
    ...
    mutate(
        nmarijuana = case_when(marijuana == "Yes" ~ 1, marijuana == "No" ~ 0)
    ) %>%
...
```{r create-x-values}
to_predict <- as.data.frame(tibble(
    year = c(0, 0, 2, 2),
    treatment = c(1, 0, 1, 0),
