...
With a sample of n = 200, do we have enough power to detect:

- The regression effect of `F` on `y` (0.5)?
...
- Factor loadings: % Sig ≈ 1.00 (all detected), but coverage is poor (0.40–0.82) — estimates are biased away from the population value of 1.00
- Regression y ON F = 0.3: % Sig ≈ 0.621 (below the 0.80 threshold)
- Residual variances: coverage for X4 is very low (0.134), indicating systematic bias in its estimate
- The M.S.E. for the regression (0.105) is high relative to the effect size

Conclusion: n = 40 is insufficient — the regression effect is underpowered and several parameters show poor coverage. We need a larger sample.

:::

## What If We Increase the Sample Size? {.smaller}
...
- Factor loadings: power improves slightly (0.80–0.93)
- Regression y ON F = 0.3: power ≈ 0.38 (still well below threshold)

At n = 50, we gain little for the regression effect — the small effect size of 0.3 is hard to detect.
...
- Factor loadings: power ≈ 1.000
- Regression y ON F = 0.3: power ≈ 0.82

At n = 200, we finally cross the 0.80 power threshold for the regression effect.
...
- Factor loadings: power ≈ 1.000
- Regression y ON F = 0.3: power ≈ 0.99

At n = 500, everything is detected with near certainty — but is this sample size realistic for your study?
...
- The Mplus `.inp` files for this example are available in the `montecarlo/` folder:
  - `power_sem.inp` (n = 200)
  - `power_sem_n50.inp` (n = 50)
  - `power_sem_n500.inp` (n = 500)
:::
:::
:::
